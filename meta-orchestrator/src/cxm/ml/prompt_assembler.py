...
""",
    ...
    def _format_system_context(self, context: Dict, user_prompt: str = "") -> str:
        ...
        if context.get('ai_sessions'):
            ai_sessions = context['ai_sessions']
            for source, data in ai_sessions.items():
                if 'recent_prompts' in data:
                    relevant_prompts = []
                    user_prompt_lower = user_prompt.lower()
                    user_words = set(re.findall(r'\b\w{4,}\b', user_prompt_lower))
                    
                    for p in data['recent_prompts']:
                        p_lower = p.lower()
                    ...
                    if relevant_prompts:
                        parts.append(f"\n**Active Session: {source.replace('_', ' ').title()}**")
                        if 'session_id' in data:
                            parts.append(f"- ID: `{data['session_id']}`")
                        parts.append("- Relevant Prompts:")
                        for p in relevant_prompts:
                            parts.append(f"  > \"{p}\"")
                elif data.get('status') == 'found':
                    parts.append(f"- Detected AI Tool: {source.replace('_', ' ').title()}")
