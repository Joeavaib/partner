...
def handle_ask(args):
    ...
    if Confirm.ask("\nSoll dieser Prompt nun in der RAG-Engine angereichert werden?", default=True):
        ...
        try:
            ...
            clipboard_msg = f"\n[bold green]✓[/bold green] {_('cli.ask.copied')}"
        except Exception as e:
            clipboard_msg = f"\n[yellow]! {_('cli.ask.copy_error')}: {e}[/yellow]"

        # Display the prompt cleanly for copying
        console.print("\n[bold green]✨ GEREINIGTER MEISTERPROMPT:[/bold green]")
        console.print("-" * 60)
        console.print(result['enhanced_prompt'])
        console.print("-" * 60)
        
        console.print(f"\n[bold green]✓[/bold green] Gespeichert in: [cyan]{output_file}[/cyan]")
        console.print(clipboard_msg)
        console.print(_('cli.ask.copy_hint'))
...
def handle_init(args):
    ...
    console.print(f"\n[bold blue]{_('cli.init.welcome')}[/bold blue]\n")
    console.print(f"✓ {_('cli.init.setup_done')}")
    console.print(f"  - {_('cli.init.workspace_created')}: [cyan]{config.get('workspace')}[/cyan]")
    console.print(f"  - {_('cli.init.config_created')}: [cyan]{config.config_path}[/cyan]")
    
    console.print(f"\n[bold yellow]{_('cli.init.next_steps')}:[/bold yellow]")
    console.print(f"  {_('cli.init.index_hint')}")
    console.print(f"  {_('cli.init.ask_hint')}\n")
...
def main():
    ...
    # init command
    subparsers.add_parser("init", help="Initialize CXM environment and default configuration")
    ...
    # ui command
    subparsers.add_parser("ui", help="Start the interactive CXM Dashboard")
    
    # If no arguments, print help or start UI
    if len(sys.argv) == 1:
        from cxm.ui import CXMUI
        config = Config()
        ui = CXMUI(get_workspace())
        ui.main_loop()
        return
    ...
    if args.command == "init":
        ...
        from cxm.ui import CXMUI
        ui = CXMUI(get_workspace(args.project, args.github))
        ui.main_loop()
    else:
        parser.print_help()
